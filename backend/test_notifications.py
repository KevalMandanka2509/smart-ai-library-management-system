...
# Force the live server to simulate emails and SMS by updating system_settings
db.system_settings.update_one(
    {"key": "smtp_config"},
    {"$set": {"value": {"host": "localhost", "port": 25, "username": "", "password": "", "from_email": "test@localhost", "tls": False}}},
    upsert=True
)
db.system_settings.update_one(
    {"key": "sms_config"},
    {"$set": {"value": {"provider": "mock", "api_key": "", "api_secret": "", "sender_id": "MOCK"}}},
    upsert=True
)
API_URL = "http://localhost:8000/api/v1"
...
def run_workflow(book_id, admin_token, expect_email, expect_sms):
    # Clear logs
    db.email_logs.delete_many({"recipient_email": "testnotif@example.com"})
    db.sms_logs.delete_many({"phone": "9998887776"})
    ...
    # Poll for async task
    for _ in range(20):
        if db.email_logs.count_documents({"recipient_email": "testnotif@example.com", "template_name": "issue_confirmation"}) > 0:
            break
        time.sleep(0.5)
    ...
    # Poll for async task
    for _ in range(20):
        if db.email_logs.count_documents({"recipient_email": "testnotif@example.com", "template_name": "return_confirmation"}) > 0:
            break
        time.sleep(0.5)
    ...
    # Verify status (could be simulated, sent, or failed depending on live server env)
    if expect_email:
        log = db.email_logs.find_one({"recipient_email": "testnotif@example.com", "template_name": "issue_confirmation"})
        assert log["status"] in ["simulated", "sent"], f"Unexpected email status: {log['status']}"
